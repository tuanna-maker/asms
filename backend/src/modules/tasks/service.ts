...
export async function createTaskService(payload: {
  ...
  return prisma.task.create({
    data: {
      ...
      priority: payload.priority ? (payload.priority as any) : undefined,
      status: payload.status ? (payload.status as any) : undefined,
      type: payload.type ? (payload.type as any) : undefined,
      ...(payload.progress !== undefined ? { progress: payload.progress } : {}),
    },
  });
}

export async function updateTaskService(id: string, payload: any) {
  const resolvedId = await resolveTaskId(id);
