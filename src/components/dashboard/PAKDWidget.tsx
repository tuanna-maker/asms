...
const PAKDWidget = ({ data }: PAKDWidgetProps) => {
  return (
    <FullscreenWrapper>
      <div className="rounded-xl bg-card p-5 shadow-sm border border-border/50 h-full flex flex-col">
        <div className="flex items-center gap-3 mb-4">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-accent/10 text-accent">
            <Wallet className="h-5 w-5" />
          </div>
          <h3 className="font-semibold text-card-foreground">Theo dõi PAKD</h3>
          <span className="ml-auto text-2xl font-bold text-card-foreground">{data.length}</span>
        </div>
            ...
            return (
              <div key={pakd.name}>
                <div className="flex justify-between text-sm mb-1">
                  <span className="font-medium text-card-foreground">{pakd.name}</span>
