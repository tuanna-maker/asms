...
import { buildRevenueLayouts } from "@/components/dashboard/dashboardLayouts";
...
const RevenueTab = ({ data, contracts = [] }: RevenueTabProps) => {
  ...
  const defaultActive = useMemo(
    () => ["stats", "chart-revenue", "pie-revenue", "trend", "table"],
    [],
  );
  const { activeWidgetIds, addWidget } = useDashboardActiveWidgets("revenue-dashboard", defaultActive);
  ...
  return (
    <>
      <DashboardGrid
        widgets={widgets}
        storageKey="revenue-dashboard"
        buildDefaultLayouts={buildRevenueLayouts}
